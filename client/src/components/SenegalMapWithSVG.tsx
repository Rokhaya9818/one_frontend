...
import { Users, Bird, AlertTriangle, Bug, X } from "lucide-react";
...
export default function SenegalMapWithSVG({ 
  ...
  const handleRegionClick = (region: string | null, event?: React.MouseEvent) => {
    if (event && region) {
  ...
  const maxCases = Math.max(
    ...safeData.map(d => getCasesForLayer(d)),
    1
  );
      ...
      <CardContent>
        ...
        <div className="relative w-full bg-blue-50 rounded-lg border border-blue-200 overflow-hidden">
          ...
          >
            ...
            {selectedRegionData && tooltipPosition && (
              <foreignObject
              ...
              >
                <div className="bg-white rounded-lg shadow-lg border-2 border-blue-500 p-3">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-bold text-sm text-gray-900">{selectedRegion}</h4>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <span className="text-gray-600">FVR Humain:</span>
                      <span className="ml-1 font-semibold">{selectedRegionData.fvr_humain.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">FVR Animal:</span>
                      <span className="ml-1 font-semibold">{selectedRegionData.fvr_animal.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Grippe Aviaire:</span>
                      <span className="ml-1 font-semibold">{selectedRegionData.grippe_aviaire.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Malaria:</span>
                      <span className="ml-1 font-semibold">{selectedRegionData.malaria.toLocaleString()}</span>
                    </div>
                    <div className="col-span-2 mt-1 pt-2 border-t border-gray-200">
                      <span className="text-gray-600">Total:</span>
                      <span className="ml-1 font-bold text-blue-600">{selectedRegionData.total_cases.toLocaleString()} cas</span>
                    </div>
                  </div>
                </div>
              </foreignObject>
